...
def main():
    ...
    write_movie(tf, q1f, q2f, mode='q', path=os.path.join(MOV_DIR, "q_prime.mp4"),
                x_axis=x_axis, y_axis=y_axis)
    write_movie(tf, psi1f, psi2f, mode='psi', path=os.path.join(MOV_DIR, "psi.mp4"),
                x_axis=x_axis, y_axis=y_axis)
    print("Done.")
...
def write_movie(t, f1, f2, mode, path, x_axis, y_axis, fps=15):
    ...
    anim = animation.FuncAnimation(fig, update, frames=len(t), interval=1000 / fps,
                                   blit=False)
    try:
